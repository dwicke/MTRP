...
rates = {'singleCon':.0625, 'fouragentCon':.0625, 'jumpshipExp':.05, 'fouragentoneNeighborhood':.25, 'fouragentfourneighborhoodSpreadout':.0625, 'oneagentoneneighborhood':.0625, 'OverlapFourAgentFourNeighborhood':.25, 'disaster1':.25, 'disaster2':.1875, 'sixtyfouragents':.8 }
areas = {'singleCon':1600 ,'fouragentCon':6400, 'jumpshipExp': 6400, 'fouragentoneNeighborhood':1600, 'fouragentfourneighborhoodSpreadout':1600, 'oneagentoneneighborhood':1600, 'OverlapFourAgentFourNeighborhood':3600, 'disaster1':3600, 'disaster2':3200, 'sixtyfouragents':102400 }
numAgents = {'singleCon':1, 'fouragentCon':4,'jumpshipExp':4, 'fouragentoneNeighborhood':4, 'fouragentfourneighborhoodSpreadout':1, 'oneagentoneneighborhood':1, 'OverlapFourAgentFourNeighborhood':4, 'disaster1':4, 'disaster2':4, 'sixtyfouragents':64 }
...
myTitle = {'singleCon':'Single Agent Contiguous Space Bias-Variance Plot', 'fouragentCon':'Four Agents 80x80 Area', 'jumpshipExp':'Four Agents 80x80', 'fouragentoneNeighborhood':'Four Agents $\lambda = .25$ A = 40x40', 'fouragentfourneighborhoodSpreadout':'Four Agents Serperate Regions', 'oneagentoneneighborhood':'One Agent $\lambda = .0625$ A = 40x40', 'OverlapFourAgentFourNeighborhood':'Four Agents Piecewise PPP', 'disaster':'Four Agents with Time Varying PPP',  'sixtyfouragents':'Sixty Four Agents' }


intervalToIndex = {'8':0, '9':1, '10':2, '11':3, '12':4, '13':5, '14':6, 
					'40':0,'45':1, '50':2, '55':3, '60':4, '65':5, '70':6}

incrementToIndex = {'0.0':0, '1.0E-4':1, '0.001':2, '0.01':3, '0.1':4, '1.0':5, '5.0':6}
xlabels = [0.0, 0.0001, 0.001, 0.01, 0.1, 1.0, 5.0]
		...
		if fuelOrNoFuel == 'NoFuel':
			for regularOrSudden in os.listdir('{}{}/{}/'.format(startDir, experiments, fuelOrNoFuel)):
				...
				for interval in os.listdir('{}{}/{}/{}/'.format(startDir, experiments, fuelOrNoFuel,regularOrSudden)):
					for method in os.listdir('{}{}/{}/{}/{}'.format(startDir, experiments, fuelOrNoFuel,regularOrSudden,interval)):
						if 'bounty' in method:
							...
							if 'Fair' in method:
								...
								#print("interval = {} increment = {} mean fairness = {} squared bias = {}".format(interval, method.split('_')[1], meanFairness, bias[intervalToIndex[interval]][incrementToIndex[method.split('_')[1]]]))
							elif 'Var' in method:
								# print(interval)
								# print(method)
								meanVar = np.mean(np.loadtxt('{}{}/{}/{}/{}/{}'.format(startDir, experiments, fuelOrNoFuel,regularOrSudden,interval,method)))
								variance[intervalToIndex[interval]][incrementToIndex[method.split('_')[1]]] = meanVar
								#print("interval = {} increment = {} mean var = {}".format(interval, method.split('_')[1], meanVar))
					...
					#plt.plot(totalError[intervalToIndex[interval]], label=interval)
					#plt.show()
				x = np.arange(0,7)
				print(x)
