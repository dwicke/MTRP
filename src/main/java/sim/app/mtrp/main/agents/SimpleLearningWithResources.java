...
public class SimpleLearningWithResources extends LearningAgent {
    ...
    boolean updatedResourceUsage = false;
    ...
    public SimpleLearningWithResources(MTRP state, int id) {
        ...
        resourceUsage = new QTable(state.getNumResourceTypes(), 1, resourceLearningRate, 0.0, state.random, state.maxMeanResourcesNeededForType, 0);
    ...
    double getUtility(Task t) {
        ...
        return ( confidenceSuccess *  (t.getBounty()+ getNumTimeStepsFromLocation(t.getLocation()) + state.getJobLength() - getCost(t))) / getNumTimeStepsFromLocation(t.getLocation());
    }
    ...
    public boolean buySellTaskResources(Depo nearestDepo) {
        ...
        for (Resource r : nearestDepo.getResources()) {
            int numShouldBuy = (int) resourceUsage.getQValue(r.getResourceType(), 0) - myResources[r.getResourceType()];
        ...
        for (Resource r : nearestDepo.getResources()) {
            int numShouldBuy = Math.min((int) r.getCurQuantity(), (int) Math.min(bounty / nearestDepo.getResourceCost(r.getResourceType()), (int) resourceUsage.getQValue(r.getResourceType(), 0) - myResources[r.getResourceType()]));
            if (numShouldBuy > 0) {
