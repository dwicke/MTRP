...
do
    ...
    do
        ...
        java -jar MTRP-1.0-SNAPSHOT.jar -repeat 4 -parallel 16 -for ${numIterations[i]} -ignoreJob 1 -y /home/dwicke/tmp/fouragentCon/NoFuel --numAgents 4 --numNeighborhoods ${numNeighborhood[i]} --simWidth 80 --simHeight 80 --agentType 4 --fuelCapacity 1500000 --hasRandomness true --numDepos 4 --fuelCost 0 --timestepsTilNextTask ${rate[i]} --jobLength ${jobLen[i]} --bountyIncrement ${increment[j]} --numJobTypes 1 -gl bounty -ls ${numIterations[i]} -es 1
    done
