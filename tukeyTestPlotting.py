...
title = 'Sudden Tasks'
expType = 'suddentasks'
files = ['/home/dfreelan/tmp/{}/0/allTimeResults.txt'.format(expType),
    '/home/dfreelan/tmp/{}/5/allTimeResults.txt'.format(expType),
    '//home/dfreelan/tmp/{}/10/allTimeResults.txt'.format(expType), 
    '/home/dfreelan/tmp/{}/15/allTimeResults.txt'.format(expType)]

colors = ['blue', '#9370DB', 'orange', 'yellow']

def get_data(files):
    data = []
    agent_means = [[] for i in range(4)]
    agent_errors = [[] for i in range(4)]
    for file in files:
...
def plot_bar_graphs(ax, means, errors, min_value=5, max_value=25, nb_samples=4):
    ...
    ax.legend(['NN','Auction','SimpleJump','Simple'])
    ax.set_title(title)
