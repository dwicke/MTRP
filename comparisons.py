...
# 		data = np.loadtxt("/home/dfreelan/tmp/{}/{}/allTimeResults.txt".format(i, joblength), dtype={'names': ('mean', 'group'), 'formats': ('f4', 'S20')})
...
# 	for joblength in [0, 5, 10, 15]:
# 		print("starting to show results for {} with job length {}".format(i, joblength))
# 		data = np.loadtxt("/home/dfreelan/tmp/{}/{}/allBountyResults.txt".format(i, joblength), dtype={'names': ('mean', 'group'), 'formats': ('f4', 'S20')})
...
colors = ['blue', '#9370DB', 'orange', 'yellow']
...
for i in range(5):
	for joblength in [10]:
		...
		data = np.loadtxt("/home/dfreelan/tmp/{}/{}/graphTimeResults.txt".format(exp[i], joblength), dtype={'names': ('time', 'mean', 'group'), 'formats': ('f4', 'f4', 'S20')})
		...
		if i != "death":
			plt.plot(data['time'][600001:4800000], data['mean'][600001:800000], label="NN", color=colors[0])
